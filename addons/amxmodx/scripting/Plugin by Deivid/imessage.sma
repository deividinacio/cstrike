...
public infoMessage()
{
    ...
    if(contain(g_Messages[g_Current], "%username%") == -1)
    {
    ...
    {
        new clientname[32], message[384]
        for(new i = 1; i <= g_MaxPlayers; i++)
        {
            if(!is_user_connected(i))
                continue
            
            copy(message, 380, g_Messages[g_Current])
            
            get_user_name(i, clientname, 31)
            replace_all(message, 380, "%username%", clientname)
