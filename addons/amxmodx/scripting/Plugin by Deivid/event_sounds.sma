...
public plugin_init()
{
	// REGISTERS PLUG-IN
	//
	register_plugin("Event Sounds", QS_VERSION, "HATTRICK (HTTRCKCLDHKS) | >>> Deivid <<< Server");

	register_clcmd("say sounds", "xAtivarDesativar")
	register_clcmd("say_team sounds", "xAtivarDesativar")
	
	// REGISTERS CONSOLE VARIABLE
	//
	new pCVar = register_cvar("advanced_event_sounds", QS_VERSION, FCVAR_SERVER | FCVAR_EXTDLL | FCVAR_UNLOGGED | FCVAR_SPONLY);
...
//
public xAtivarDesativar(Player)
{
	
	if (g_bConnected[Player] && !g_bBOT[Player] && !g_bHLTV[Player])
	{
	
		client_print(Player, print_chat, ">> Sons de evento foi %s.", g_bDisabled[Player] ? "Ativado" : "Desativado");
...
public client_putinserver(Player)
{
	...
	if (!g_bBOT[Player] && !g_bHLTV[Player])
		set_task(6.0, "EventSoundsPrint", Player);
}

// PRINTS INFORMATION TO PLAYER
//
public EventSoundsPrint(Player)
{
	// ONLY IF CONNECTED
	//
	if (g_bConnected[Player])
		client_print(Player, print_chat, ">> Digite 'sounds' para desativar sons de eventos.");
}
...
__Load()
{
	...
	//
	add(File, charsmax(File), "/event_sounds.ini");
