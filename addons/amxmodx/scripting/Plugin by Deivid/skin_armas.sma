...
new maxplayers
new bool:g_weapons[33]

public plugin_init()
{
	register_plugin("Weapon Model + Sound Replacement",VERSION,"1.0")
	...
	register_clcmd("say_team /skin", "admin")
}
...
public admin(id)
{
	if (get_user_flags(id) & ADMIN_LEVEL_H)
        ...
        {
               client_print_color(id, "!g>> !yPara !tHabilitar/Desabilitar Skin !yVoce precisa ser !gADM !yAdministrador!")
        }
	
}

public desativar_armas(id)
{

	new menu = menu_create("\r[\w >>> SERVER DEIVID <<< SKIN DE ARMAS ? \r]\r", "menu_wybierz")
    
    menu_additem(menu, "\wHABILITAR + Reiniciar Partida", "1", 0)
    menu_additem(menu, "\wDESABILITAR + Reinciar Partida", "2", 0)
...
public menu_wybierz(id, menu, item)
{
    ...
    { 
       case 1 :
		if(g_weapons[id] == false)
		{
			g_weapons[id] = true;
			       
			client_print_color(id, "!g[%s] !yModels de armas ativadas.",wmPrefix);
			client_print_color(0, "!g[%s] !tJogador: !y%s !tATIVOU as skins de armas.",wmPrefix, szName);
			client_print_color(id, "!g[%s] !yPara acessar o menu novamente digite !g/skin.",wmPrefix);
			server_cmd("sv_restartround 1")
		}	
       case 2 :
       
		 if(g_weapons[id] == true)
		{
			g_weapons[id] = false;
			client_print_color(id, "!g[%s] !yModels de armas desativadas.",wmPrefix);
			client_print_color(0, "!g[%s] !tJogador: !y%s !tDESATIVOU as skins de armas.",wmPrefix, szName);
			client_print_color(id, "!g[%s] !yPara acessar o menu novamente digite !g/skin.",wmPrefix);
			server_cmd("sv_restartround 1")
		}
...
public client_connect(id) g_weapons[id] = true;
...
public plugin_precache()
{
	...
	get_mapname(map,31)
	format(configfile,199,"%s/novas_armas_%s.ini",configsdir,map)
	...
	{
		format(configfile,199,"%s/novas_armas.ini",configsdir)
		load_models(configfile)
	}
}
			...
			if(containi(read,";")!=0 && containi(read," ")!=-1)
			{
				...
				if(file_exists(right))
				{
					if(containi(right,".mdl")==strlen(right)-4)
					{
						...
						{
							log_amx("Error attempting to precache model: ^"%s^" (Line %d of novas_armas.ini)",right,i+1)
						}
						...
						{
							log_amx("Model type(p_ / v_ / w_) unknown for model: ^"%s^" (Line %d of novas_armas.ini)",right,i+1)
						}
					}
					else if(containi(right,".wav")==strlen(right)-4 || containi(right,".mp3")==strlen(right)-4)
					{
						...
						{
							log_amx("Error attempting to precache sound: ^"%s^" (Line %d of novas_armas.ini)",right,i+1)
						}
						else
						{
					...
					{
						log_amx("Invalid File: ^"%s^" (Line %d of novas_armas.ini)",right,i+1)
					}
				}
				else
				{
					log_amx("File Inexistent: ^"%s^" (Line %d of novas_armas.ini)",right,i+1)
				}
